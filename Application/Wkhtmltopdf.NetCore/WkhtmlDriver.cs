...
namespace Wkhtmltopdf.NetCore
{
    public abstract class WkhtmlDriver
    {
        ...
        public static byte[] Convert(string wkhtmlPath, string switches, string html, string headerPath, string footerPath)
        {
            // switches:
            ...
            //headerPath = "/Users/pragashrajarathnam/projects/bistec/DYMK/OneCommonPortal/Application/Backend/ROCBN/Api/bin/Debug/netcoreapp3.1/header.html";
            //footerPath = "/Users/pragashrajarathnam/projects/bistec/DYMK/OneCommonPortal/Application/Backend/ROCBN/Api/bin/Debug/netcoreapp3.1/footer.html";
            //switches = string.Format("--header-html {0} --footer-html {1} -q " + switches + " -", headerPath, footerPath);

            if (!string.IsNullOrEmpty(headerPath))
            {
                switches += string.Format(" --header-html {0} --disable-smart-shrinking", headerPath);
            }

            if (!string.IsNullOrEmpty(footerPath))
            {
                switches += string.Format(" --footer-html {0} ", footerPath);
            }
            switches += " -q -";
